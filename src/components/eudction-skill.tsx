            ...
            >
                <CardContent className='p-6 flex flex-col items-center justify-between h-full'>
                    ...
                    <div className='mt-4 px-3 py-1 border border-primary text-primary rounded-full text-sm'>
                        {data.dateStarted || data.startDate} -{" "}
                        {data.dateEnded || data.endDate}
                    </div>
...
export default function EducationAndSkills({
    ...
    const nextEducation = () => {
        setActiveEducationIndex((prev) =>
            prev === null || prev === education?.length - 1 ? 0 : prev + 1
        );
    };

    const prevEducation = () => {
        setActiveEducationIndex((prev) =>
            prev === null || prev === 0 ? education?.length - 1 : prev - 1
        );
    };

    const filteredSkills = skillsSet?.filter(
        (category: Category) => filter === "All" || category?.title === filter
    );
