...
import { Github, Linkedin, Twitter, Instagram, ChevronDown, ExternalLink, Mail, Download, Facebook } from "lucide-react"
...
export default function HeroSection({ pageInfo, socials }: HeroProps) {
  ...
  const filter = useTransform(scrollYProgress, [0, 1], ["blur(0px)", "blur(5px)"])
    ...
    >
      ...
      >
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-8 md:gap-16 w-full">
          ...
          <motion.div
            ...
            animate={{ opacity: loadedSections >= 1 ? 1 : 0, x: loadedSections >= 1 ? 0 : -50 }}
              ...
              >
                <div className="w-full h-full overflow-hidden rounded-full bg-background p-1">
                  <div className="w-full h-full overflow-hidden rounded-full bg-background relative">
                    {pageInfo?.heroImage && (
                      <Image
                        src={urlFor(pageInfo.heroImage).url() || "/placeholder.svg"}
          ...
          >
            <div className="space-y-2">
              <motion.div className="text-reveal-container" initial={{ opacity: 1 }} animate={{ opacity: 1 }}>
                <motion.div
                ...
                >
                  <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold text-muted-foreground">
                    <span className="text-gradient-animate from-secondary via-tertiary to-primary">
                      {pageInfo?.role || "Developer & Designer"}
                    </span>
                  </h2>
                </motion.div>
              </motion.div>
            ...
            >
              <Button
                ...
                onMouseEnter={() => handleMouseOver("Contact Me")}
                onMouseLeave={handleMouseLeave}
              >
              ...
              <Button
                ...
                onMouseEnter={() => handleMouseOver("Download Resume")}
                onMouseLeave={handleMouseLeave}
              >
            ...
            >
              {socials?.map((social: any, i: number) => (
                <motion.div
                  ...
                  initial="hidden"
                  animate={loadedSections >= 3 ? "visible" : "hidden"}
                >
                  <Link
                    href={social.url}
                    target="_blank"
                    ...
                    onMouseEnter={() => handleMouseOver(`Visit ${social.title}`)}
                    onMouseLeave={handleMouseLeave}
                  >
                    <Button
                      variant="outline"
                    ...
                    >
                      {socialIcons[social.title.toLowerCase() as keyof typeof socialIcons] || (
      ...
      >
        ...
        <motion.div
          ...
          transition={{ duration: 15, repeat: Number.POSITIVE_INFINITY, repeatType: "loop" }}
        />
        ...
        <motion.div
          ...
          transition={{ duration: 15, repeat: Number.POSITIVE_INFINITY, repeatType: "loop", delay: 2 }}
        />
      </motion.div>
    </section>
  )
